...
void OffensiveDataExtractor::init_file(DEState &state) {
    ...
    std::string dir = "/home/pedro/robocin/simulation-2d-cpp/py-scripts/action-generation/db_offensive_dt_extractor";
    strftime(buffer, sizeof(buffer), "%Y-%m-%d-%H-%M-%S", timeinfo);
    std::string str(buffer);
    std::string rand_name = std::to_string(RobocinPlayer::playerPort);
