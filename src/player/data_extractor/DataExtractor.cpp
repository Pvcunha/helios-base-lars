...
#define ADD_ELEM2(vec, value) vec.push_back((float)value)
...
DataExtractor::DataExtractor() : last_update_cycle(-1)
{
}
...
std::vector<float> DataExtractor::get_last_features(const rcsc::PlayerAgent *agent, const CooperativeAction &first_action, bool update_shoot)
{
    ...
    {
        extract_output(wm,
                       first_action.category(),
                       first_action.targetPoint(),
                       first_action.targetPlayerUnum(),
...
void DataExtractor::init_file(const rcsc::WorldModel &wm)
{
    ...
    str += "_" + std::to_string(wm.self().unum()) + "_E" + rand_name + ".csv";
    ...
    // Cycle and BALL
    std::string header = "timehash,cycle,ball_pos_x,ball_pos_y,ball_pos_r,ball_pos_t,ball_kicker_x,ball_kicker_y,ball_kicker_r,ball_kicker_t,ball_vel_x,ball_vel_y,ball_vel_r,ball_vel_t,";
...
void DataExtractor::extract_ball(const rcsc::WorldModel &wm)
{
    if (wm.ball().posValid())
    {
        ADD_ELEM2(last_features, convertor_x(wm.ball().pos().x));
        ADD_ELEM("p_x", convertor_x(wm.ball().pos().x));
        ADD_ELEM2(last_features, convertor_y(wm.ball().pos().y));
        ADD_ELEM("p_y", convertor_y(wm.ball().pos().y));
        ADD_ELEM2(last_features, convertor_dist(wm.ball().pos().r()));
        ADD_ELEM("p_r", convertor_dist(wm.ball().pos().r()));
        ADD_ELEM2(last_features, convertor_angle(wm.ball().pos().th().degree()));
        ADD_ELEM("p_t", convertor_angle(wm.ball().pos().th().degree()));
        ADD_ELEM2(last_features, convertor_dist_x(wm.ball().rpos().x));
        ADD_ELEM("kicker_x", convertor_dist_x(wm.ball().rpos().x));
        ADD_ELEM2(last_features, convertor_dist_y(wm.ball().rpos().y));
        ADD_ELEM("kicker_y", convertor_dist_y(wm.ball().rpos().y));
        ADD_ELEM2(last_features, convertor_dist(wm.ball().rpos().r()));
...
DataExtractor::sort_players(const rcsc::WorldModel &wm)
{
    ...
    for (int i = 1, j = 0; i <= 11 && j <= 1; i++)
    {
        ...
        if (player == NULL || player->isGhost() || player->unum() < 1 || !player->pos().isValid() || !player->vel().isValid())
            continue;
        if (j == 0)
        {
                                            ...
                                            DataSide side)
{
    ...
    {
        double angle_diff = fabs((players_in_area[i] - player->pos()).th().degree() - (players_in_area[i - 1] - player->pos()).th().degree());
...
void DataExtractor::extract_drible_angles(const WorldModel &wm)
{
    ...
    for (double angle = -180; angle < 180; angle += delta)
    {
        ...
        ADD_ELEM2(last_features, convertor_dist(min_opp_dist));
